...
def preprocessing(iteration=0, ignore_lower_bits=0, keysize=1):
    ...
    with open(f"eval_data_{keysize}_noprune/seed_{ignore_lower_bits}_{iteration}.txt", "w") as f:
    ...
    os.system(f"mv key.hex eval_data_{keysize}_noprune/key_{ignore_lower_bits}_{iteration}.hex")
...
def postprocessing(iteration=0, ignore_lower_bits=0, keysize=1):
    print("[-] Cleaning up...")
    os.system(f"mv main eval_data_{keysize}_noprune/main_{ignore_lower_bits}_{iteration}")
    os.system(f"mv cftrace.csv eval_data_{keysize}_noprune/cftrace_{ignore_lower_bits}_{iteration}")
    os.system(f"mv dftrace.csv eval_data_{keysize}_noprune/dftrace_{ignore_lower_bits}_{iteration}")
...
if __name__ == "__main__":
    for keysize in [2]:
        print(f"\nKEYSIZE = {keysize}")
        os.makedirs(f"eval_data_{keysize}_noprune", exist_ok=True)
        with open(f"eval_data_{keysize}_noprune/statistics.csv", "w") as f:
        ...
        for bits in [0]:
            ...
            for it in range(10):
                ...
                with open(f"eval_data_{keysize}_noprune/key_{bits}_{it}.hex", "rb") as f:
                    key = f.read()
                incorrect_bytes = 0
                ...
                print(f"")
                with open(f"eval_data_{keysize}_noprune/statistics.csv", "a") as f:
